...
|---|---|
| v0.8.12 | **Visible "Checking for updates…" gate at launch.** The desktop launch update check was silent (`setTimeout(checkForUpdates(false),1500)`); now `launchUpdateCheck()` shows a centred **`#upd-splash`** card (dimmed backdrop + animated CSS ring spinner; `prefers-reduced-motion` stops the spin; hidden in print) while the updater is queried (`Promise.race` w/ 8s timeout so offline never traps; ~600ms min so a fast check doesn't flicker), auto-dismisses, and prompts only on an actual update. Install flow refactored into shared **`promptAndInstall(update)`** (confirm → `downloadAndInstall` % → `relaunch`), reused by the manual Help ▸ Check for Updates path. No-op in a plain browser. `updatecheck` smoke → **539/539**; build smoke asserts the gate → 65/65. Mirrored in 3D v0.0.16. |
| v0.8.11 | **Edge reference guides + shared-stitch visual fixes (assembly-schema v4→v5).** **(A) Edge reference guides (TODO 4):** a new seam **`type:'guide'`** — a directional, non-joining alignment annotation (no stitching/fold, never length-flagged), so multiple edges can be declared to align **to** one target edge (the T-pocket case in `MultiEdge-To-SingleEdge.png`), giving Leather Studio 3D something to position against. Reuses the whole seam pipeline (Seam tool picking, Assembly panel, undo, validate). **members[0] is the target**; the editor tags it "target" + a **⤒ make-target** button on other members (`setSeamTarget`); Type dropdown gains "Guide (align only)"; `SEAM_TYPE_BADGE.guide='⇲'`. Canvas: target edge drawn solid+heavier, source edges dashed, each with a dashed **direction arrow** (world-space, `worldEdgeMid`) pointing at the target. `sharedSeamForEdge`/`edgeStitched` already gate on `type==='stitch'`, so guides never touch stitching; `seamLengthIssues` skips guides. **(B) Shared-stitch ("Stitch as one seam") visuals:** (1) a screen-only **running-stitch connector** (`sharedSeamRunsForShape` → `<polyline class="seam-aid">`) threads each shared member's holes in order, so the seam reads as ONE continuous run and a shared edge is distinguishable from a plain stitched edge even in Select; (2) **corner-ownership fix in `stitchRect`** — a shared seam now OWNS every corner it touches (skip the perpendicular edge's start hole when the previous edge is a seam, skip the far-corner push when the next edge is a seam), so a mismatched edge vs seam margin no longer leaves two holes ~a margin apart (a visible double); the equal-margin hole count is unchanged. Schema: `assembly.version` 4→5, `'guide'` added to `normSeam`/`setSeamType`; `.lpd` file v15 unchanged (additive enum). `seam` smoke +card-holder coincidence/no-double + guide round-trip; full **532/532**. |
| v0.8.10 | **Per-edge stitch toggle list (UX discoverability).** First item of the queued `frontend-design`+`ui-language` UX audit. The Stitching panel now shows a visible per-edge list under "Stitch all edges" (`renderEdgeStitchList`/`#edge-stitch-list`): one row per edge with a live checkbox — rect edges read Top/Right/Bottom/Left (`edgeLabel`), paths Edge N; shared-seam-owned edges show a disabled box + orange **seam** tag. Two-way canvas sync: hovering a row glows the edge (`hoverEdgeRow`→`S.hoverEdge`), clicking selects it (`selectEdgeRow`), the checkbox toggles just that edge (`toggleEdgeFromList`). Guarded against no-op rewrites (`box._h`) so hover doesn't churn the row under the cursor. Replaces the old single canvas-select-only `#edge-stitch-row`. **(#2) Empty-state invitation:** `#no-sel-msg` is now an actionable prompt — title + sub + three clickable draw-tool buttons (Rect/Pen/Text → `setTool`); `updatePropsPanel` tailors the copy (blank doc "Start your pattern", populated "No shape selected"). `.ns-*` token-based styles. **(#3) Per-tool status hints:** `TOOL_HINTS` map → right-aligned `#st-hint` span set by `applyToolChrome` (every tool teaches itself; Pen keeps its panel too). **(#4) Unified toolbar icons:** per-button Unicode glyphs → inline SVG at one 24-viewBox/18px size (`currentColor`); T/? stay letters (`.t-ico-letter`); destructive **Delete** pulled out of the tool column. **(#5) Shortcut key badges:** faint `.t-key` corner badge on V/R/A/B/P/S. **(#6) Empty-canvas prompt:** `#canvas-empty` french-stitch-framed "Draw your first piece" card on a blank doc (reuses `frenchBorder`; frame sized to the MEASURED text card so slits don't overlap copy; screen-only, `updateCanvasEmpty` in `renderContent`) + **`centerOnArtboard()`** frames the active artboard centred on entry/new-doc (was top-left). **Default colour cycles the palette:** `nextShapeColor()`/`_colorCycle` over `SHAPE_COLORS`, applied in `addShape`+pen-finish+`makeTextShape` (clones keep theirs; explicit pick overrides; pen previews in editor chrome so colour is set once at finish). **(#7)** status-bar hierarchy (stateful tool/snap brighter, passive telemetry faint); **(#8)** Settings moved out of Edit → always-visible menubar gear `#mb-settings` (`.mb-icon-btn`); **(#9)** Assembly panel callout pointing to Leather Studio 3D. HTML/CSS/JS only, no save-format change. `peredge`/`emptystate`/`toolux`/`color` smoke; full **518/518**. **All 9 audit items + colour-cycling done.** |
...
| v0.8.1 | **Seam authoring — Assembly panel + per-seam editor (Steps 3-4).** New collapsible **Assembly** `.p-sec` (shown in the Seam tool or when seams exist): **create** a seam from the pending picks (`createSeamFromSelection`, pushHist), a seam **list** (stable per-seam colour chip via `seamColor`, name, type badge ✄/⟋/●, member count, delete), and a per-seam **editor** (`seamEditorHTML`): name (unique, auto-suffixed via `uniqueSeamName`), type stitch/fold/glue, order, allowance, and a **members** list with locate (◎ → select shape+edge) / remove (✕) + add-picked-edges (`addPicksToSeam`). Committed seams render on canvas in the seam colour + name label (`class="seam-aid"` → stripped from print + SVG/PNG export); clicking a member edge in the Seam tool selects its seam. New state `S.activeSeam` (panel selection, resets on doc-swap/load, cleared if its seam is pruned). All edits undoable; everything round-trips in the `.lpd`. `seam` smoke 11→37; full **435/435**. | 
| v0.8.0 | **Seam / assembly model — schema v15 + Seam tool (first cut).** New top-level `assembly` (per-doc, in `DOC_KEYS` + undo snapshot): `{version, seams:[{id,name,type:stitch\|fold\|glue,order,allowance,members:[{shape,edge,reversed?}]}], folds:[{id,shape,a,b,angle}]}`. Save bumped **v14→v15** (absent → empty on load; `normAssembly`/`normSeam` normalise; `validateSeams()` prunes refs to deleted shapes / out-of-range edges / circles + rebuilds the derived `S._seamMap` "id:edge"→seam; runs on load/undo/tab-swap). Optional per-piece `sh.thickness` (mm). New **Seam tool** (toolbar `⋈`, key **S**): `hitAnyEdge` picks edges across ANY shape into `S.seamSel` (amber + index badges; cyan hover), Esc clears; grouping into named seams is the next step (Assembly panel). Consumed read-only by Leather Studio 3D — contract in `MD files/SEAM-MODEL.md`. `seam` smoke feature + assembly round-trip asserts; full **409/409**. | 
